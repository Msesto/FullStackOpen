        ...
        phonebook
          ...
          .catch(error => {
            ...
            if(error.response.data.error === "ValidationError"){
              setNotification(`Input fields are invalid, name must contain at least 3 characters and a valid number has atleast 8 numbers`)
            } else { setNotification(`${newAddition.name} was removed from the server.`) };
            setTimeout(() => {
              setNotification(null)
            }, 5000)
            phonebookService
